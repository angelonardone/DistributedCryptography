...
namespace GeneXus.Programs.distributedcrypto {
   public class decryptjson : GXProcedure
   {
      ...
      public string executeUdp( string aP0_encryptedText ,
                                ...
                                out string aP3_clearText )
      {
         execute(aP0_encryptedText, aP1_encryptedKey, aP2_IV, out aP3_clearText, out aP4_error);
         return AV9error ;
      }
      ...
      protected override void ExecutePrivate( )
      {
         ...
         {
            GXt_char2 = AV9error;
            new GeneXus.Programs.distributedcrypto.aesdecryptstring(context ).execute(  AV13encryptedText, out  AV12clearText,  AV8encryptionKey,  AV11IV, out  GXt_char2) ;
            AV9error = GXt_char2;
            if ( String.IsNullOrEmpty(StringUtil.RTrim( AV9error)) )
            {
            }
